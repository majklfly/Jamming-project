...
const fetchUserData = async (accessToken) => {
  ...
  const spotifySearch = dispatch => {
      return async (term, token) =>  {
        ...
        response.status === 200 ? dispatch({ type: "get_tracks", payload: response.data.tracks.items}) : console.log(response.status)
  }
}

export const { Context, Provider } = createDataContext(
    fetchReducer, {fetchUserData, getToken, spotifySearch }, initialState
  );